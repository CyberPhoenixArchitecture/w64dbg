...
w64dbg is a tiny tool debugging live user-mode and kernel-mode code for 64-bit Windows OS and dumping out its stack frames. It can read executable's __PDB__ format itself and read 's __DWARF__ format generated by GNU C/C++ Compiler in backend.
...
After download build binaries or build it yourself, run **setup.bat** to ensure working environment is fine.
...
w64dbg cannot handles more than **MAX_THREAD** and **MAX_DLL** (which is 64 and 16 defined [here](src/main.c#L15)) as it is abnormal for a process to overcome these limits. If you want to adjust them, just change the definition in source files and build them. By the way, w64dbg will not print out more than 100 thread context frames as the only way to reach that unbelievable number is infinite recursive.
